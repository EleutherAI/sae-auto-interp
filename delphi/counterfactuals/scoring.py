...
def expl_given_generation_score(scorer, scorer_tokenizer, completions_path, device):
    ...
    assert (
        not os.path.exists(out_path) or "debug" in out_path
    ), f"Output path {out_path} already exists"

    # get KV cache for the scoring few-shot prompt
    scorer_prompt = get_scorer_surprisal_prompt(
        few_shot_prompts[0],
        few_shot_generations[0],
        few_shot_explanations[0],
        few_shot_prompts,
        few_shot_explanations,
        few_shot_generations,
    )
    scorer_fs_prefix = scorer_prompt[: scorer_prompt.rfind(scorer_separator)]
    scorer_fs_ids = scorer_tokenizer(
        scorer_fs_prefix, return_tensors="pt"
    ).input_ids.to(device)
        ...
        for explanation in record["explanations"]:
            ...
            for i in range(len(record["completions"])):
                for name, completion in record["completions"][i]["completions"].items():
                    ...
                    scorer_expl_ids = scorer_tokenizer(
                        expl, return_tensors="pt", add_special_tokens=False
                    ).input_ids.to(device)
                    scorer_input_ids = torch.cat(
                        [scorer_prompt_ids, scorer_expl_ids], dim=1
                    )
                    ...
                    with torch.inference_mode():
                        loss = scorer(
                            ...
                            labels=labels,
                            use_cache=True,
                            past_key_values=scorer_fs_kv,
                        ).loss
                        # HF averages over the sequence length, so we undo that
                        surprisals[name].append(
                            loss.item()
                            * (scorer_input_ids.shape[1] - scorer_prompt_ids.shape[1])
                        )

            surprisals = {k: np.array(v) for k, v in surprisals.items()}
            surprisals_by_explanation[explanation] = surprisals
            delta_conditional_entropy_by_explanation[explanation] = (
                surprisals["clean"] - surprisals["intervened"]
            ).mean()
            delta_conditional_entropy_sems_by_explanation[explanation] = (
                surprisals["clean"] - surprisals["intervened"]
            ).std(ddof=1) / np.sqrt(len(surprisals["intervened"]))

        best_explanation = max(
            delta_conditional_entropy_by_explanation,
            key=lambda x: delta_conditional_entropy_by_explanation[x],
        )
        record.update(
            {
                ...
                "max_delta_conditional_entropy": max(
                    delta_conditional_entropy_by_explanation.values()
                ),
