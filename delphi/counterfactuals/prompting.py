...
class ExplainerInterventionExample:
    ...
    def text(self) -> str:
        tokens_str = ", ".join(
            f"'{tok}' (+{round(p, 3)})"
            for tok, p in zip(self.top_tokens, self.top_p_increases)
        )
...
def get_explainer_prompt(
    ...
    few_shot_examples: list[ExplainerNeuronFormatter] | None = None,
) -> str:
    prompt = (
...
def get_scorer_surprisal_prompt(
    prompt,
    generation,
    explanation,
    few_shot_prompts=None,
    few_shot_explanations=None,
    few_shot_generations=None,
    return_explanation_start=False,
) -> str:
    if few_shot_explanations is not None:
        assert few_shot_generations is not None and few_shot_prompts is not None
        assert (
            len(few_shot_explanations)
            == len(few_shot_generations)
            == len(few_shot_prompts)
        )
        few_shot_prompt = (
            "\n\n".join(
                get_scorer_surprisal_prompt(txt, gen, expl)
                for txt, expl, gen in zip(
                    few_shot_prompts, few_shot_explanations, few_shot_generations
                )
            )
            + "\n\n"
        )
