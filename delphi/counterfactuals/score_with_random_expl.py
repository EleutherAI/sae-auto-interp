...
def main(
    expl_path: Path | str,
    explainer_name: str = "meta-llama/Meta-Llama-3.1-8B",
    device: str = "cuda",
):
    save_path = (
        Path(str(expl_path).replace("re=False", "re=True")).parent / "generations.json"
    )
    ...
    if Counter(all_expls).most_common(1)[0][1] / len(all_expls) > 0.25:
        warnings.warn(
            "More than 25% of the explanations are the same. Shuffling may not cause enough change in the explanations."
        )

    # shuffle the explanations column
    random_explanations_df["explanations"] = random.sample(
        random_explanations_df["explanations"].tolist(), k=len(random_explanations_df)
    )
