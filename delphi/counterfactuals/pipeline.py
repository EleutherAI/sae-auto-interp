...
from ..latents import LatentDataset, LatentLoader
...
def get_latent_loader(
    ...
    latent_cfg = LatentConfig(
        width=width, n_splits=5, max_examples=100000, min_examples=200
    )
    experiment_cfg = ExperimentConfig(
        ...
        n_examples_test=0,
        n_examples_train=(n_train + n_test) // n_quantiles,
...
def get_avg_kl(
    feat_idx, feat_layer, ids_s, intervention_strength, clamp_value, get_subject_logits
):
    avg_kl = 0.0
    for ids in ids_s:
        intervened_logps = get_subject_logits(
            ids,
            feat_layer,
        ...
        clean_logps = get_subject_logits(
            ids,
            feat_layer,
...
def tune_intervention_strength(
    ...
    for i in range(10):
        ...
        avg_kl = get_avg_kl(
            ...
            clamp_value=None,
            get_subject_logits=get_subject_logits,
        )
        ...
        log_strs = np.log10(
            np.array(sorted(list(avg_kls.keys()), key=lambda k: avg_kls[k]))
        )
        ...
        print(
            f"Binary search for intervention strength did not converge after {i + 1} iterations"
        )
        print(
            f"Using {intervention_strength} with KL {avg_kl} when target is {kl_threshold}"
        )
        print(f"{avg_kls=}")
...
def main(
    ...
    save_dir = (
        PATH_ROOT / f"counterfactual_results/{run_prefix}_{subject_name.split('/')[-1]}"
    )
    ...
    if raise_if_exists:
        assert (
            not (save_dir / "generations_scores.json").exists() or "debug" in run_prefix
        ), f"Save path {save_path} already exists"
    with open(config_save_path, "w") as f:
        json.dump(config, f)

    loader = get_latent_loader(
        feat_layer, n_feats, n_train, n_test, n_quantiles, sae_width, latents=latents
    )

    subject = AutoModelForCausalLM.from_pretrained(
        subject_name, torch_dtype=torch.bfloat16
    ).to(device)
    ...
    if latents == "sae":
        ...
        pt_params = {
            k: torch.from_numpy(v).to(device).to(torch.bfloat16)
            for k, v in params.items()
        }
        sae = (
            JumpReLUSAE(params["W_enc"].shape[0], params["W_enc"].shape[1])
            .to(device)
            .to(torch.bfloat16)
        )
        ...
        params = torch.load(
            PATH_ROOT
            / f"cache/gemma_topk/layer_{feat_layer}/width_{sae_width}/k_{k}_seed_42/params.pt"
        )
        sae = Autoencoder(
            sae_width,
            params["encoder.weight"].shape[1],
            activation=ACTIVATIONS_CLASSES["TopK"](k=k),
            normalize=False,
            tied=False,
        )
    ...
    def get_subject_logits(
        ...
        subject_layers[layer].register_forward_hook(
            partial(
                intervene_on_feat,
                intervention_strength=intervention_strength,
                clamp_value=clamp_value,
    ...
    def generate_with_intervention(
        ids, layer, intervention_strength=None, clamp_value=None, feat_idx=None
    ):
        for l in range(len(subject_layers)):
            subject_layers[l]._forward_hooks.clear()
        ...
        subject_layers[layer].register_forward_hook(
            partial(
                ...
                intervention_strength=intervention_strength,
                clamp_value=clamp_value,
                position=slice(intervention_start, None),
                feat_idx=feat_idx,
            )
        )
        with torch.inference_mode():
        ...
        return subject_tokenizer.decode(out[0]).removeprefix(
            subject_tokenizer.bos_token
        )
    ...
    if save_path.exists():
        ...
        all_explainer_examples = [
            [get_first_activating_text(e) for e in pool[:n_train]] for pool in pools
        ]
        all_scorer_examples = [
            [get_first_activating_text(e) for e in pool[n_train:]] for pool in pools
        ]
        ...
        for iter, (record, scorer_examples) in enumerate(
            tqdm(
                zip(loader, all_scorer_examples),
                desc="Tuning intervention strengths",
                total=len(all_scorer_examples),
            )
        ):
            garbage_collect()
            ...
            if zero_ablate:
                ...
                intervention_strength, avg_kls = tune_intervention_strength(
                    feat_idx, feat_layer, ids_s, kl_threshold, get_subject_logits
                )
        ...
        for iter, (record, scorer_examples) in enumerate(
            ...
            for ids, max_act in scorer_examples:
                ...
                completions.append(
                    {
                        "text": prompt.removeprefix(subject_tokenizer.bos_token),
                ...
                for name, strength in [
                    ("clean", None),
                    ("intervened", all_results[iter]["intervention_strength"]),
                ]:
                    cv = 0 if name == "intervened" and zero_ablate else None
                    completions[-1]["completions"][name] = generate_with_intervention(
                        ids,
                        feat_layer,
            ...
            all_results[iter].update(
                {
                    ...
                    "scorer_examples": [
                        subject_tokenizer.decode(ids) for ids, _ in scorer_examples
                    ],
        ...
        for iter, (record, explainer_examples) in enumerate(
            tqdm(
                zip(loader, all_explainer_examples),
                desc="Running interventions for explainer",
                total=len(all_explainer_examples),
            )
        ):
            garbage_collect()
            feat_idx = record.latent.latent_index
            ...
            for ids, act in explainer_examples:
                clean_logits = get_subject_logits(
                    ids,
                    feat_layer,
                    intervention_strength=None,
                    clamp_value=None,
                ...
                intervened_logits = get_subject_logits(
                    ids,
                    feat_layer,
                    intervention_strength=all_results[iter]["intervention_strength"],
                    clamp_value=0 if zero_ablate else None,
                    feat_idx=feat_idx,
                )
                top_probs = (
                    intervened_logits.softmax(dim=-1) - clean_logits.softmax(dim=-1)
                ).topk(n_tokens_per_explainer_example)
            ...
            all_results[iter].update(
                {
                    ...
                    "explainer_examples": [
                        subject_tokenizer.decode(ids) for ids, _ in explainer_examples
                    ],
                }
            )
            if (iter - 1) % 10 == 0:
        ...
        for iter, row in enumerate(tqdm(all_results, desc="Generating explanations")):
            ...
            explainer_input_ids = explainer_tokenizer(
                explainer_prompt, return_tensors="pt"
            ).input_ids.to(device)
            with torch.inference_mode():
            ...
            explanations = [
                explainer_tokenizer.decode(sample).split("\n")[0].strip()
            ...
            all_results[iter].update(
                {
                    ...
                    "explainer_prompts": [
                        example.prompt for example in intervention_examples
                    ],
