...
processed_dir = "/share/u/caden/sae-auto-interp/neighbors"

for i in range(0,12,2):
    tensor = submodule_dict[i].ae.autoencoder._module.decoder.weight
    cos_sim = cos(tensor)
    top = torch.topk(cos_sim[:500], k=10)

    torch.save(top, os.path.join(processed_dir, f"{submodule_dict[i]._module_path}.pt"))
