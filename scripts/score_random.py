...
for layer in layers:
    ...
    for record in tqdm(records):
        try:
            random_feature = record.feature
            while str(random_feature)==str(record.feature):
                random_idx = random.randint(0,len(records)-1)
                random_feature = records[random_idx].feature
            random_explanation = load_explanation("saved_explanations/gpt2_split_top_200_20_all_20",random_feature)["result"]
        ...
        explanations.append(random_explanation)
        
    


client = get_client("local", "casperhansen/llama-3-70b-instruct-awq", base_url="http://localhost:8000/v1")
scorer = FuzzingScorer(client,model.tokenizer,batch_size=5, echo=False,n_few_shots=5)

output_dir = "scores/gpt2_random"
asyncio.run(
    execute_model(
        scorer, 
        random_inputs,
