...
async def process_cache(
    ...
    client = Offline(
        ...
        max_memory=0.8,
        max_model_len=11_000, # Explainer models context length - must be able to accomodate the longest set of examples
        num_gpus=8
    )
    ...
    # fuzzing_scorer_pipe = process_wrapper(
    #     FuzzingScorer(client, tokenizer=dataset.tokenizer, log_prob=True),
    #     preprocess=scorer_preprocess,
    #     postprocess=partial(scorer_postprocess, score_dir=fuzz_scores_path),
    # )
    # detection_scorer_pipe = process_wrapper(
    #     DetectionScorer(client, tokenizer=dataset.tokenizer, log_prob=True),
    #     preprocess=scorer_preprocess,
    #     postprocess=partial(scorer_postprocess, score_dir=detection_scores_path),
    # )
...
def populate_cache(
    ...
    submodule_to_sae, hooked_model = load_gemma_autoencoders(
        model, ae_layers=layers, average_l0s={10: 47}, size="131k", type="res"
    )
...
def build_df(path: Path, target_modules: list[str], range: Tensor):
    ...
    for score_type in scores_types:
        ...
        for module in target_modules:
            for score_file in list(score_type_path.glob(f"*{module}*")) + list(score_type_path.glob(f".*{module}*")):
...
def plot_line(df, visualizations_path: Path, target_modules: list[str]):
    ...
    for score_type in df["score_type"].unique():
        ...
        plot_data = []
        for module in target_modules:
            values = df[(df["score_type"] == score_type) & (df["module"] == module)]["probability"]
            if len(values) > 0:
                ...
                plot_data.extend([{"x": x, "density": d, "module": module} 
        ...
        if len(plot_data) > 0:
            ...
            fig.write_image(visualizations_path / f"autointerp_probabilities_{score_type}.pdf", format="pdf")

        # Create density curves for accuracies
        plot_data = []
        for module in target_modules:
            values = df[(df["score_type"] == score_type) & (df["module"] == module)]["accuracy"]
            if len(values) > 0:
                kernel = stats.gaussian_kde(values)
                x_range = np.linspace(values.min(), values.max(), 200)
                density = kernel(x_range)
        ...
        if len(plot_data) > 0:
            ...
            fig.write_image(visualizations_path / f"autointerp_accuracies_{score_type}.pdf", format="pdf")
...
async def main():
    ...
    if not glob(str(latents_path / ".*")) + glob(str(latents_path / "*")) or 'cache' in args.overwrite:
        populate_cache(latents_path, sae_layers, tokenizer)
    else:
        print(f"Files found in {latents_path}, skipping cache population...")

    if not glob(str(scores_path / ".*")) + glob(str(scores_path / "*")) or 'scores' in args.overwrite:
        await process_cache(latents_path, explanations_path, scores_path, modules, tokenizer, torch.arange(1, 1000))
    else:
        print(f"Files found in {scores_path}, skipping...")

    if not glob(str(visualizations_path / ".*")) + glob(str(visualizations_path / "*")) or 'visualize' in args.overwrite:
