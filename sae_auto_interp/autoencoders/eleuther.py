...
def load_random_eai_autoencoders(
    model: Any,
    ae_layers: List[int],
    cfg: SaeConfig,
    seed: int = 42,
    ...
    for layer in ae_layers:
        ...
        submodule = model.model.layers[layer]
        sae = Sae(submodule.mlp.gate_proj.in_features, cfg, device=DEVICE, dtype=model.dtype, decoder=False)
        ...
        topk = TopK(cfg.k, postact_fn=ACTIVATIONS_CLASSES["Identity"]())
        def _forward(sae, x):
            encoded = sae.pre_acts(x)
            return topk(encoded)
        
        submodule.ae = AutoencoderLatents(
            sae, partial(_forward, sae), width=sae.encoder.out_features
        )
