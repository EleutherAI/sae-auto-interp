...
def main(expl_path: Path | str, explainer_name: str, device: str):
    # NOTE: this is kind of a hack
    save_path = Path(str(expl_path).replace("re=True", "re=False").replace("generations.json", "generations_scores.json"))
    random_explanations_df = pd.read_json(expl_path)
    all_expls = [e for l in random_explanations_df["explanations"].tolist() for e in l]
    print(f"{Counter(all_expls)=}")
